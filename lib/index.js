...
function _receive(data) {
  if (data) {
    ...
    for (var i in data.component.methods) {
      ...
      value = i + '()';
      methods.push({ value: value, name: name, params: data.component.methods[i].params });
    }
  ...
  return function () {
    ...
    _createClass(videPluginPromptVue, [{
      ...
      value: function mappingTemplate(position) {
        ...
        if (matches) {
          // match vue component
          var component = line.slice(matches.index + 1).match(/^[\w\-\$]+/);
          if (component) {
          ...
          if (matches) {
            matches = prevFlagment.match(/[\w\-\$]+$/);
            var event = line.slice(matches.index).match(/^[\w\-\$]+/);
      ...
      value: function mappingScript(position) {
        ...
        matches = prevFlagment.match(/[\w\-\$]+$/);
        if (matches) {
          var str = line.slice(matches.index);
          matches = str.match(/^([\w\-\$]+)\(/);
          if (matches) {
            var func = matches[1];
